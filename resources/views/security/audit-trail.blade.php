                    ...
                    <div class="card-body">
                        <form method="GET" action="{{ route('security.audit-trail') }}" class="row g-3">
                                ...
                                <select name="user_filter" class="form-select" onchange="this.form.submit()">
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            {{ request('user_filter') === (string) $user->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" name="ip_search" class="form-control" placeholder="Cari IP address..."
        ...
        <div class="table-container">
                    ...
                    <tbody>
                        @forelse($auditTrails as $trail)
                            <tr class="align-middle">
                                <td>
                                    <span
                                        class="badge bg-light text-dark">{{ $trail->created_at->format('d M Y') }}</span><br>
                                    <small class="text-muted">{{ $trail->created_at->format('H:i:s') }}</small>
                                </td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="avatar avatar-sm me-2 bg-primary rounded-circle d-flex align-items-center justify-content-center text-white"
                                ...
                                <td>
                                    @if ($trail->old_values || $trail->new_values)
                                        <button type="button" class="btn btn-sm btn-outline-info" data-bs-toggle="modal"
                                            data-bs-target="#detailModal{{ $trail->id }}">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="row mb-3">
                                                            ...
                                                            <div class="col-md-6">
                                                                <strong>Waktu:</strong>
                                                                {{ $trail->created_at->format('d M Y H:i:s') }}<br>
                                                        ...
                                                        @if ($trail->old_values)
                                                            <h6 class="mb-3">
                                                                <i
                                                                    class="bi bi-arrow-left-circle text-danger me-2"></i>Nilai
                                                                Lama
                                                            </h6>
                                                        ...
                                                        @if ($trail->new_values)
                                                            <h6 class="mb-3">
                                                                <i
                                                                    class="bi bi-arrow-right-circle text-success me-2"></i>Nilai
                                                                Baru
                                                            </h6>
            ...
            <div class="mt-4 d-flex justify-content-between align-items-center">
                <div class="text-muted">
                    Menampilkan <strong>{{ $auditTrails->count() }}</strong> dari
                    <strong>{{ $auditTrails->total() }}</strong> records
                </div>
                <div>
                    {{ $auditTrails->links() }}
                </div>
