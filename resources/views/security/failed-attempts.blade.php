            ...
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <p class="text-muted mb-0">Monitor percobaan login yang gagal dan identifikasi ancaman brute force
                        </p>
                    </div>
                                            ...
                                            <td>
                                                <div class="progress" style="height: 20px;">
                                                    <div class="progress-bar bg-danger"
                                                        style="width: {{ min($trend->count * 5, 100) }}%"></div>
                                                </div>
                                            </td>
                                        </tr>
                                    @empty
                    ...
                    <div class="card-body">
                        <form method="GET" action="{{ route('security.failed-attempts') }}" class="row g-3">
                            <div class="col-md-3">
                                ...
                                <input type="text" name="ip_search" class="form-control" placeholder="e.g., 192.168.1.1"
                                    value="{{ request('ip_search') }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Cari Email/Username</label>
                                <input type="text" name="email_search" class="form-control"
                                    placeholder="e.g., user@example.com" value="{{ request('email_search') }}">
                            </div>
                            <div class="col-md-3">
                    ...
                    <div class="card-body">
                                ...
                                <tbody>
                                    @forelse($failedAttempts as $attempt)
                                        <tr class="align-middle">
                                            <td>
                                                <span
                                                    class="badge bg-light text-dark">{{ $attempt->created_at->format('d M Y') }}</span><br>
                                                <small
                                                    class="text-muted">{{ $attempt->created_at->format('H:i:s') }}</small>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <code
                                                    class="bg-danger bg-opacity-10 p-2 rounded text-danger">{{ $attempt->ip_address }}</code>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                <!-- Modal -->
                                                <div class="modal fade" id="detailModal{{ $attempt->id }}"
                                                    tabindex="-1">
                                                    <div class="modal-dialog modal-lg">
                                                        <div class="modal-content border-danger border-3">
                                                            <div
                                                                class="modal-header bg-danger bg-opacity-10 border-danger">
                                                                <h5 class="modal-title text-danger">
                                                                    <i class="bi bi-lock-fill me-2"></i>Failed Login
                                                                    Attempt Details
                                                                </h5>
                                                            ...
                                                            <div class="modal-body">
                                                                <div class="row mb-3">
                                                                    <div class="col-md-6">
                                                                        ...
                                                                        <code>{{ $attempt->resource_id }}</code><br>
                                                                        <strong>IP Address:</strong> <code
                                                                            class="bg-danger bg-opacity-10">{{ $attempt->ip_address }}</code><br>
                                                                        <strong>Severity:</strong> <span
                                                                            class="badge bg-{{ $attempt->severity === 'critical' ? 'danger' : 'warning' }}">{{ ucfirst($attempt->severity) }}</span>
                                                                    </div>
                                                                    <div class="col-md-6">
                                                                        <strong>Waktu:</strong>
                                                                        {{ $attempt->created_at->format('d M Y H:i:s') }}<br>
                                                                        <strong>Status:</strong> <span
                                                                            class="badge bg-danger">{{ ucfirst($attempt->status) }}</span><br>
                                                                        <strong>Event Type:</strong>
                                                                        {{ ucfirst($attempt->event_type) }}
                                                                    </div>
                                                                </div>
                                                                    ...
                                                                    <div class="bg-light p-3 rounded">
                                                                        <table class="table table-sm mb-0">
                                                                            <tr>
                                                                                ...
                                                                                <td><span
                                                                                        class="badge bg-danger">{{ $attempt->metadata['attempt_number'] ?? 'N/A' }}</span>
                                                                                </td>
                                                                            </tr>
                                                                            <tr>
                                                                                <td><strong>Threshold:</strong></td>
                                                                                <td>{{ $attempt->metadata['threshold'] ?? 'N/A' }}
                                                                                    attempts</td>
                                                                            </tr>
                                                                            <tr>
                                                                                <td><strong>Time Window:</strong></td>
                                                                                <td>{{ $attempt->metadata['time_window'] ?? 'N/A' }}
                                                                                </td>
                                                                            </tr>
                                                                        </table>
                                                                    </div>
                                                                ...
                                                                @if ($attempt->user_agent)
                                                                    ...
                                                                    <small
                                                                        class="text-muted d-block text-break">{{ $attempt->user_agent }}</small>
                                                                @endif
                                                            </div>
                        ...
                        <div class="mt-4 d-flex justify-content-between align-items-center">
                            <div class="text-muted">
                                Menampilkan <strong>{{ $failedAttempts->count() }}</strong> dari
                                <strong>{{ $failedAttempts->total() }}</strong> attempts
                            </div>
                            <div>
